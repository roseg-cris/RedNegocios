...
$stmt = $pdo->prepare("
    SELECT p.*, sc.nombre AS subcategoria_nombre,
...
<head>
	...
	<title>RedNegocios | Mis productos</title>
                ...
                <div class="ps-page__content row">
                    ...
                    <div class="container">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            <a href="../../panel/productos/crear_producto_negocio.php" class="ps-btn">+ Nuevo producto</a>
                        </div>

                        <?php if (empty($productos)): ?>

                        <div class="alert alert-secondary">Aún no has agregado productos a tu catálogo.</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4 col-md-6 col-12 mb-4">
                                <div class="card h-100">
                                    <img src="<?= !empty($producto['imagen_principal']) ? '../../uploads/productos/' . htmlspecialchars($producto['imagen_principal']) : '../../assets/img/products/electronic/1.jpg' ?>" class="card-img-top" style="height:180px;object-fit:cover;" alt="">
                                    <div class="card-body">
                                        <h5 class="card-title"><?= htmlspecialchars($producto['nombre']) ?></h5>
                                        <p class="card-text text-muted mb-1"><?= htmlspecialchars($producto['subcategoria_nombre']) ?></p>
                                        <p class="card-text">
                                            <?php if (!empty($producto['precio_oferta'])): ?>
                                            <span class="text-muted"><del>$<?= number_format($producto['precio'], 2) ?></del></span>
                                            <strong class="text-danger">$<?= number_format($producto['precio_oferta'], 2) ?></strong>
                                            <?php else: ?>
                                            <strong>$<?= number_format($producto['precio'], 2) ?></strong>
                                            <?php endif; ?>
                                        </p>
                                        <span class="badge <?= $producto['disponibilidad'] === 'disponible' ? 'badge-success' : 'badge-secondary' ?>"><?= htmlspecialchars(ucfirst($producto['disponibilidad'])) ?></span>
                                        <span class="text-muted small ml-2">Stock: <?= (int) $producto['stock'] ?></span>
                                    </div>
                                </div>
                            </div>
                            <?php endforeach; ?>
