...
$stmt = $pdo->prepare(
    "SELECT l.created_at, p.nombre AS producto_nombre, p.precio, p.precio_oferta,
            p.disponibilidad, n.nombre AS negocio_nombre
     FROM likes_producto l
...
<head>
	...
	<title>RedNegocios | Mis Likes</title>
                     ...
                     <div class="ps-page__content row">
                        ...
                        <div class="container">
                            ...
                            <div class="ps-section__header">
                                ...
                                <h1><i class="fas fa-thumbs-up"></i> Productos que me gustan</h1>

                                <h4><?= count($misLikes) ?> like(s) dado(s)</h4>

                            </div>

                            <?php if (empty($misLikes)): ?>
                            <p class="text-muted"><em>Aún no le has dado like a ningún producto.</em></p>
                            <?php else: ?>
                            <div class="table-responsive mt-4">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        <?php foreach ($misLikes as $like): ?>
                                        <tr>
                                            <td><?= htmlspecialchars($like['producto_nombre']) ?></td>
                                            <td><?= htmlspecialchars($like['negocio_nombre']) ?></td>
                                            <td>
                                                <?php if (!empty($like['precio_oferta'])): ?>
                                                <span class="text-muted"><del>$<?= number_format((float) $like['precio'], 2) ?></del></span>
                                                $<?= number_format((float) $like['precio_oferta'], 2) ?>
                                                <?php else: ?>
                                                $<?= number_format((float) $like['precio'], 2) ?>
                                                <?php endif; ?>
                                            </td>
                                            <td><?= htmlspecialchars(date('d/m/Y', strtotime($like['created_at']))) ?></td>
                                        </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
