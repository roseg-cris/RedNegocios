...
$stmt = $pdo->prepare(
    "SELECT c.contenido, c.created_at, p.nombre AS producto_nombre, n.nombre AS negocio_nombre
     FROM comentarios c
...
<head>
	...
	<title>RedNegocios | Mis Comentarios</title>
                     ...
                     <div class="ps-page__content row">
                        ...
                        <div class="container">
                            ...
                            <div class="ps-section__header">
                                ...
                                <h1><i class="fas fa-comment"></i> Mis comentarios</h1>

                                <h4><?= count($misComentarios) ?> comentario(s) publicado(s)</h4>

                            </div>

                            <?php if (empty($misComentarios)): ?>
                            <p class="text-muted"><em>Aún no has comentado en ningún producto.</em></p>
                            <?php else: ?>
                            <div class="mt-4">
                                <?php foreach ($misComentarios as $com): ?>
                                <div class="card mb-3">
                                    <div class="card-body">
                                        <h5><?= htmlspecialchars($com['producto_nombre']) ?> <small class="text-muted">— <?= htmlspecialchars($com['negocio_nombre']) ?></small></h5>
                                        <p class="mb-1"><?= nl2br(htmlspecialchars($com['contenido'])) ?></p>
                                        <small class="text-muted"><?= htmlspecialchars(date('d/m/Y H:i', strtotime($com['created_at']))) ?></small>
                                    </div>
                                </div>
                                <?php endforeach; ?>
                            </div>
