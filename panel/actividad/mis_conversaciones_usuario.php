...
$stmt = $pdo->prepare(
    "SELECT co.id, co.created_at, n.nombre AS negocio_nombre, p.nombre AS producto_nombre,
            (SELECT contenido FROM mensajes m WHERE m.conversacion_id = co.id ORDER BY m.created_at DESC, m.id DESC LIMIT 1) AS ultimo_mensaje,
            (SELECT created_at FROM mensajes m WHERE m.conversacion_id = co.id ORDER BY m.created_at DESC, m.id DESC LIMIT 1) AS ultima_fecha
     FROM conversaciones co
     JOIN negocios n ON n.id = co.negocio_id
     LEFT JOIN productos p ON p.id = co.producto_id
     WHERE co.usuario_id = ?
     ORDER BY co.id DESC"
);
$stmt->execute([$usuarioId]);
$misConversaciones = $stmt->fetchAll();
?>
<!DOCTYPE html>
<html lang="es">
<head>
	...
	<title>RedNegocios | Mis Conversaciones</title>
        ...
        <div class="container">
            ...
            <div class="ps-section__header">
                ...
                <div class="ps-section__content">
                    ...
                    <div class="mt-3 mb-2">
                        <a href="../../panel/perfil/perfil_usuario.php"><i class="fas fa-arrow-left"></i> Volver a mi perfil</a>
                    </div>
                     ...
                     <div class="ps-page__content row">
                        ...
                        <div class="container">
                            ...
                            <div class="ps-section__header">
                                ...
                                <h1><i class="fas fa-comments"></i> Mis conversaciones</h1>

                                <h4><?= count($misConversaciones) ?> conversación(es)</h4>

                            </div>

                            <?php if (empty($misConversaciones)): ?>
                            <p class="text-muted"><em>Aún no has iniciado ninguna conversación con un negocio.</em></p>
                            <?php else: ?>
                            <div class="mt-4">
                                <?php foreach ($misConversaciones as $conv): ?>
                                <div class="card mb-3">
                                    <div class="card-body">
                                        <h5>
                                            <?= htmlspecialchars($conv['negocio_nombre']) ?>
                                            <?php if (!empty($conv['producto_nombre'])): ?>
                                            <small class="text-muted">— <?= htmlspecialchars($conv['producto_nombre']) ?></small>
                                            <?php endif; ?>
                                        </h5>
                                        <?php if (!empty($conv['ultimo_mensaje'])): ?>
                                        <p class="mb-1"><?= nl2br(htmlspecialchars($conv['ultimo_mensaje'])) ?></p>
                                        <small class="text-muted"><?= htmlspecialchars(date('d/m/Y H:i', strtotime($conv['ultima_fecha']))) ?></small>
                                        <?php else: ?>
                                        <p class="mb-1 text-muted"><em>Todavía no hay mensajes en esta conversación.</em></p>
                                        <small class="text-muted"><?= htmlspecialchars(date('d/m/Y H:i', strtotime($conv['created_at']))) ?></small>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <?php endforeach; ?>
